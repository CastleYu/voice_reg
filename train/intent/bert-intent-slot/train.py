...
def train(args):
    ...
    # 保存tokenizer
    tokenizer.save_pretrained(model_save_dir)  # 正确保存分词器
    ...
    dev_acc, intent_avg, slot_avg = dev(model, val_dataloader, device, slot_dict)
    # 在训练过程中保存模型
    save_module(model, model_save_dir)  # 正确保存模型
...
if __name__ == '__main__':
    ...
    parser.add_argument("--cuda_devices", type=str, default='1', help='set cuda device numbers')
    ...
    # training parameters
    parser.add_argument("--save_dir", type=str, default='./save_model_2', help="directory to save the model")
