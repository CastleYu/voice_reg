...
class XFYunASR(XFYun, WS):
    def __init__(self, app_id, api_key, api_secret):
        """
        初始化科大讯飞语音转文字模块
        """
        super(XFYunASR, self).__init__(host=HOST, api_key=api_key, api_secret=api_secret)
...
if __name__ == "__main__":
    ...
    file_path = r"H:\Music\ReneeRecord"
    dp = DiskPath(file_path)
    stt = XFYunASR(cfg.APP_ID, cfg.API_KEY, cfg.API_SECRET)
    for i in dp.list()
